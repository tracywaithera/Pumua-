...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#fdfcf9]">
      ...
      {/* Sidebar / Navigation */}
      <nav className="w-full md:w-64 bg-white border-b md:border-b-0 md:border-r border-[#eeeae4] p-6 flex flex-col gap-8 z-50">
        ...
        <div className="pt-6 border-t border-[#eeeae4] space-y-4">
          <div>
            <span className="text-xs font-semibold uppercase tracking-widest text-[#8e8a84] block mb-2">Your Name</span>
            <input
              type="text"
              value={userName}
              onChange={(e) => setUserName(e.target.value)}
              placeholder="Enter your name..."
              className="w-full bg-[#f5f2ed] border-none rounded-lg p-2 text-sm focus:ring-1 focus:ring-[#5A5A40]"
            />
          </div>
          ...
          <div className="pt-2">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold uppercase tracking-widest text-[#8e8a84]">Calm Music</span>
                {isMusicPlaying && (
                  <div className="flex items-end gap-0.5 h-4">
                    <div className="w-0.5 bg-[#5A5A40] waveform-bar" style={{ animationDelay: '0s' }} />
                    <div className="w-0.5 bg-[#5A5A40] waveform-bar" style={{ animationDelay: '0.2s' }} />
                    <div className="w-0.5 bg-[#5A5A40] waveform-bar" style={{ animationDelay: '0.4s' }} />
                    <div className="w-0.5 bg-[#5A5A40] waveform-bar" style={{ animationDelay: '0.1s' }} />
                  </div>
                )}
              </div>
              <button onClick={toggleMusic} className="text-[#5A5A40] hover:scale-110 transition-transform">
                {isMusicPlaying ? <Volume2 size={18} /> : <VolumeX size={18} />}
              </button>
            </div>
            <div className="w-full bg-[#f5f2ed] rounded-lg p-3 text-sm text-[#5A5A40] text-center font-medium mt-2">
              YouTube Ambience
            </div>
            {isMusicPlaying && (
              <iframe
                width="0"
                height="0"
                src={`https://www.youtube.com/embed/${AMBIENCE_VIDEO_ID}?autoplay=1&loop=1&playlist=${AMBIENCE_VIDEO_ID}`}
...
function WelcomeOverlay({ onComplete }: { onComplete: (name: string) => void }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[100] bg-[#fdfcf9] flex flex-col items-center justify-center p-8 text-center"
    >
      <motion.div
      ...
      >
        <div className="space-y-4">
          <div className="w-20 h-20 rounded-3xl bg-[#5A5A40] flex items-center justify-center text-white mx-auto shadow-2xl shadow-[#5A5A40]/30">
            <Heart size={40} />
          </div>
          <h1 className="font-serif text-5xl font-bold">Pumua</h1>
          <p className="text-[#6b6863] text-xl italic font-serif">"Just breathe."</p>
        </div>

        <div className="space-y-6">
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-widest text-[#8e8a84]">How should we address you?</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="Enter your name or initial..."
              className="w-full bg-white border border-[#eeeae4] rounded-2xl px-6 py-4 text-center text-xl focus:ring-2 focus:ring-[#5A5A40] transition-all"
          ...
          <button
            ...
            disabled={!name.trim()}
            className="w-full py-4 bg-[#5A5A40] text-white rounded-2xl font-semibold shadow-xl hover:bg-[#4a4a34] transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Enter Sanctuary
          </button>
        </div>

        <p className="text-[#8e8a84] text-sm">
...
function ReminderPopup({
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-8 right-8 z-[110] w-80 bg-white border border-[#eeeae4] rounded-3xl shadow-2xl p-6 overflow-hidden"
    >
      ...
      <button
        onClick={onClose}
        className="absolute top-4 right-4 text-[#8e8a84] hover:text-[#2d2a26] transition-colors"
      >
        <X size={18} />
      </button>
      ...
      <div className="flex flex-col gap-4">
        ...
        {reminder.type === 'breathe' && (
          <div className="pt-2">
            <p className="text-xs font-serif italic text-[#5A5A40]">Be present in this moment.</p>
          </div>
        )}
      </div>
    </motion.div>
    ...
    <button
      ...
      className={cn(
        ...
        active
          ...
          : "text-[#6b6863] hover:bg-[#f5f2ed] hover:text-[#2d2a26]"
      )}
    >
      {icon}
      {label}
    ...
    >
      <header className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-[#5A5A40] font-serif italic text-xl">Just breathe.</span>
        </div>
      </header>

      {/* Daily Dose Section */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <section className="bg-white border border-[#eeeae4] rounded-[2rem] p-8 space-y-6 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-600">
              <Sparkles size={20} />
            </div>
            <h3 className="text-xs font-bold uppercase tracking-widest text-[#8e8a84]">Daily Healing Tip</h3>
          </div>
          <div className="space-y-2">
            <h4 className="text-xl font-serif font-medium">{dailyTip.title}</h4>
            <p className="text-[#6b6863] text-sm leading-relaxed">{dailyTip.text}</p>
          </div>
        </section>

        <section className="bg-[#5A5A40] text-white rounded-[2rem] p-8 space-y-6 shadow-xl shadow-[#5A5A40]/20">
          <div className="flex items-center gap-3">
      ...
      {/* Daily Verse Card */}
      <section className="bg-[#f5f2ed] rounded-[2rem] p-10 relative overflow-hidden group">
        <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:opacity-20 transition-opacity">
          <Sparkles size={120} />
        </div>
        <div className="relative z-10 space-y-6">
          <span className="text-xs font-bold uppercase tracking-[0.2em] text-[#8e8a84]">Daily Spiritual Nourishment</span>
          <blockquote className="font-serif text-3xl md:text-4xl leading-tight text-[#2d2a26]">
            "{verse.text}"
          </blockquote>
          <cite className="block text-[#5A5A40] font-medium not-italic">— {verse.reference}</cite>
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div
          ...
          className="bg-white border border-[#eeeae4] p-8 rounded-3xl hover:border-[#5A5A40] transition-colors cursor-pointer group"
        >
          ...
          <p className="text-[#6b6863] text-sm">Instant stress relief through guided diaphragmatic breathing.</p>
        </div>
        <div
        ...
        >
          ...
          <p className="text-[#6b6863] text-sm">Professional insights grounded in neuroscience and faith.</p>
        </div>
      </div>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-bold uppercase tracking-widest text-[#8e8a84]">Active Instrumental Music</h3>
          <button className="text-sm text-[#5A5A40] font-medium hover:underline">View All Tracks</button>
        </div>
        <div className="grid grid-cols-1 gap-4">
          <div
            ...
            className={cn(
              ...
              isPlaying ? "border-[#5A5A40] shadow-md" : "border-[#eeeae4] hover:border-[#5A5A40]"
            )}
          >
            <div className={cn(
              ...
              isPlaying
                ...
                : "bg-[#f5f2ed] text-[#5A5A40] group-hover:bg-[#5A5A40] group-hover:text-white"
            )}>
              {isPlaying ? <Pause size={24} /> : <Play size={24} />}
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-lg font-medium truncate">Healing Ambience</div>
              <div className="text-xs text-[#8e8a84] uppercase tracking-wider">YouTube • Continuous</div>
            </div>
            {isPlaying && (
              <div className="flex items-end gap-1 h-6">
                <div className="w-1 bg-[#5A5A40] waveform-bar" style={{ animationDelay: '0s' }} />
                <div className="w-1 bg-[#5A5A40] waveform-bar" style={{ animationDelay: '0.2s' }} />
                <div className="w-1 bg-[#5A5A40] waveform-bar" style={{ animationDelay: '0.4s' }} />
              </div>
            )}
          </div>
        </div>
...
function BreatherView({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="h-full flex flex-col items-center justify-center p-8 space-y-12 bg-gradient-to-b from-[#fdfcf9] to-[#f5f2ed] relative"
    >
      ...
      <div className="absolute top-8 right-8 flex items-center gap-3 bg-white/50 backdrop-blur px-4 py-2 rounded-full border border-white/20">
        <div className="flex flex-col items-end">
          <span className="text-[10px] uppercase tracking-widest text-[#8e8a84] font-bold">Ambient Sound</span>
          <span className="text-xs font-medium text-[#5A5A40]">{selectedTrack.name}</span>
        </div>
        <button
          onClick={onTogglePlay}
          className="w-10 h-10 rounded-full bg-[#5A5A40] text-white flex items-center justify-center shadow-lg hover:scale-105 transition-transform"
        >
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-[#6b6863]">Calm your nervous system. Find your rhythm.</p>
      </div>
      ...
      <div className="relative flex items-center justify-center">
        ...
        >
          <span className="text-2xl font-serif text-[#2d2a26]">{phase}</span>
          <span className="text-5xl font-bold text-[#2d2a26]">{timer}</span>
        </motion.div>
        ...
        <div className="absolute inset-0 -z-10">
          <motion.div
            ...
            className="absolute inset-0 border-2 border-[#5A5A40] rounded-full"
          />
        </div>
      </div>
      ...
      <button
        ...
        className="px-12 py-4 bg-[#5A5A40] text-white rounded-full font-semibold shadow-xl hover:bg-[#4a4a34] transition-colors"
      >
        {isActive ? 'Pause Session' : 'Start Breathing'}
      </button>

      <div className="max-w-md text-center text-sm text-[#8e8a84] italic">
...
function TherapistView({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        // Auto-speak back if it's a short response or user preference
        // For this demo, we'll let user click the play button
      }
    } catch (error) {
      console.error(error);
    } finally {
  ...
  return (
    ...
    >
      <div className="p-8 border-b border-[#eeeae4] flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-[#8e8a84] uppercase tracking-widest">Neuroscience • Psychology • Faith</p>
          </div>
        </div>
        <div
          onClick={() => setShowTopUp(true)}
          className="flex items-center gap-2 bg-[#f5f2ed] px-4 py-2 rounded-full cursor-pointer hover:bg-[#eeeae4] transition-colors"
        >
          <Coins size={16} className="text-amber-600" />
          <span className="text-sm font-bold text-[#5A5A40]">{credits} Credits</span>
        </div>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-6">
        {messages.length === 0 && (
          <div className="text-center py-20 space-y-4">
            <div className="w-16 h-16 bg-[#f5f2ed] rounded-full flex items-center justify-center mx-auto text-[#5A5A40]">
              <Sparkles size={32} />
            </div>
            <h3 className="text-xl font-serif">Sema, {userName || 'rafiki'}?</h3>
            <p className="text-[#6b6863] max-w-sm mx-auto">
          ...
          <div key={i} className={cn("flex", m.role === 'user' ? "justify-end" : "justify-start")}>
            <div className={cn(
              ...
              m.role === 'user'
                ...
                : "bg-white border border-[#eeeae4] text-[#2d2a26] rounded-tl-none shadow-sm"
            )}>
              ...
              {m.role === 'model' && (
                <button
                  ...
                  className={cn(
                    "absolute -right-12 top-0 p-2 rounded-full bg-white border border-[#eeeae4] text-[#5A5A40] opacity-0 group-hover:opacity-100 transition-opacity disabled:opacity-50",
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="bg-white border border-[#eeeae4] p-4 rounded-3xl rounded-tl-none animate-pulse">
              <div className="flex gap-1">
                <div className="w-2 h-2 bg-[#8e8a84] rounded-full" />
                <div className="w-2 h-2 bg-[#8e8a84] rounded-full" />
                <div className="w-2 h-2 bg-[#8e8a84] rounded-full" />
              </div>
            </div>
      ...
      {isRecording && (
        <div className="absolute bottom-32 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur-md border border-[#eeeae4] px-6 py-3 rounded-full shadow-2xl flex items-center gap-3 z-50">
          <div className="w-3 h-3 bg-red-500 rounded-full animate-pulse" />
          <span className="text-sm font-medium text-[#5A5A40]">Tubonge is listening...</span>
          <span className="text-xs text-[#8e8a84] italic">"{input.slice(-30)}..."</span>
        </div>
      )}

      <div className="p-8 bg-white border-t border-[#eeeae4] relative">
        <div className="max-w-3xl mx-auto flex gap-4 items-center">
          <input
            ...
            className="flex-1 bg-[#f5f2ed] border-none rounded-2xl px-6 py-4 focus:ring-2 focus:ring-[#5A5A40] transition-all"
          />
          ...
          <motion.button
            ...
            className={cn(
              ...
              isRecording ? "bg-red-500 text-white scale-125" : "bg-[#f5f2ed] text-[#5A5A40] hover:bg-[#eeeae4]"
            )}
          >
            <Mic size={24} />
          </motion.button>
          ...
          <button
            ...
            className="bg-[#5A5A40] text-white px-8 py-4 rounded-2xl font-semibold hover:bg-[#4a4a34] transition-colors disabled:opacity-50"
          >
            Send
          </button>
        </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <label className="text-xs font-bold uppercase tracking-widest text-[#8e8a84]">M-Pesa Number</label>
            <input
              type="tel"
              value={phoneNumber}
              onChange={(e) => setPhoneNumber(e.target.value)}
              placeholder="0712345678"
              className="w-full bg-[#f5f2ed] border-none rounded-xl px-4 py-3 focus:ring-2 focus:ring-[#5A5A40]"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              ...
              className="p-6 border border-[#eeeae4] rounded-2xl hover:border-[#5A5A40] transition-all text-center group"
            >
              <div className="text-xl font-bold text-[#5A5A40]">20 KES</div>
              <div className="text-xs text-[#8e8a84]">40 Credits</div>
            </button>
            <button
              ...
              className="p-6 border border-[#eeeae4] rounded-2xl hover:border-[#5A5A40] transition-all text-center group"
            >
              <div className="text-xl font-bold text-[#5A5A40]">50 KES</div>
              <div className="text-xs text-[#8e8a84]">100 Credits</div>
            </button>
          </div>
        </div>
        ...
        {isProcessing ? (
          ...
          <p className="text-xs text-[#8e8a84] text-center">
  ...
  return (
    ...
    >
      <header>
        ...
        <p className="text-[#6b6863]">Externalize your thoughts. Release the weight.</p>
      </header>
      ...
      <div className="space-y-6">
        <textarea
          ...
          className="w-full h-64 bg-white border border-[#eeeae4] rounded-[2rem] p-8 focus:ring-2 focus:ring-[#5A5A40] transition-all resize-none shadow-sm"
        />
        <div className="flex justify-end">
          <button
            onClick={handleSave}
            className="px-10 py-4 bg-[#5A5A40] text-white rounded-full font-semibold shadow-lg hover:bg-[#4a4a34] transition-colors"
          >
            Save Reflection
          </button>
        </div>
      ...
      {savedEntries.length > 0 && (
        <div className="space-y-6">
          <h3 className="text-xs font-bold uppercase tracking-widest text-[#8e8a84]">Previous Reflections</h3>
          <div className="grid gap-4">
            {savedEntries.map((e, i) => (
              <div key={i} className="bg-[#f5f2ed] p-6 rounded-2xl">
                <span className="text-xs font-medium text-[#5A5A40]">{e.date}</span>
                <p className="mt-2 text-[#2d2a26] line-clamp-2">{e.text}</p>
              </div>
            ))}
          </div>
        </div>
    ...
    return (
      ...
      >
        <button
          ...
          className="flex items-center gap-2 text-[#5A5A40] font-medium"
        >
          <ArrowLeft size={18} /> Back to Library
        </button>

        <div className="absolute top-8 right-8 flex items-center gap-3 bg-white/80 backdrop-blur px-4 py-2 rounded-full border border-[#eeeae4] shadow-sm z-10">
          <div className="flex flex-col items-end">
            <span className="text-[10px] uppercase tracking-widest text-[#8e8a84] font-bold">Reading Ambience</span>
            <div className="text-xs font-medium text-[#5A5A40] mt-1 text-right">
              YouTube Ambience
            </div>
          </div>
          <button
            ...
            className={cn(
              ...
              isPlaying ? "bg-[#5A5A40] text-white" : "bg-white text-[#5A5A40] border border-[#eeeae4]"
            )}
          >
            {isPlaying ? <Volume2 size={18} /> : <VolumeX size={18} />}
          </button>
        ...
        <header className="space-y-4 pt-12">
          <span className="text-xs font-bold uppercase tracking-widest text-[#8e8a84]">{selectedArticle.category}</span>
          <h2 className="font-serif text-5xl leading-tight">{selectedArticle.title}</h2>
        </header>
        <div className="text-lg leading-relaxed text-[#2d2a26] space-y-6">
  ...
  return (
    ...
    >
      <header>
        ...
        <p className="text-[#6b6863]">Knowledge is the first step toward healing.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {ARTICLES.map(a => (
          <div
            ...
            className="bg-white border border-[#eeeae4] rounded-3xl overflow-hidden hover:shadow-xl transition-all cursor-pointer group"
          >
            <div className="p-8 space-y-4">
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#8e8a84]">{a.category}</span>
              <h3 className="text-xl font-serif group-hover:text-[#5A5A40] transition-colors">{a.title}</h3>
              <p className="text-sm text-[#6b6863] line-clamp-3">{a.excerpt}</p>
              <div className="pt-4 flex items-center text-[#5A5A40] text-sm font-semibold">
  ...
  return (
    ...
    >
      <header>
        ...
        <p className="text-[#6b6863]">Your body is a temple. Honor it with gentle motion and mindful insights.</p>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {VIDEOS.map(video => (
          <div
            ...
            className="bg-white border border-[#eeeae4] rounded-3xl overflow-hidden hover:shadow-xl transition-all group cursor-pointer"
          >
            <div className="aspect-video relative overflow-hidden">
              <img src={video.thumbnail} alt={video.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              <div className="absolute inset-0 bg-black/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <div className="w-12 h-12 rounded-full bg-white/90 flex items-center justify-center text-[#5A5A40]">
            ...
            <div className="p-6 space-y-2">
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#8e8a84]">{video.category}</span>
              <h3 className="text-lg font-serif group-hover:text-[#5A5A40] transition-colors">{video.title}</h3>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8">
        <div className="bg-[#f5f2ed] p-8 rounded-[2rem] space-y-4">
          ...
          <div className="pt-2 flex items-center gap-2 text-[#5A5A40] font-medium text-sm">
            Learn more about brain health <ChevronRight size={16} />
          </div>
        </div>
        <div className="bg-white border border-[#eeeae4] p-8 rounded-[2rem] space-y-4">
  ...
  return (
    ...
    >
      <header className="space-y-6">
        <div className="flex items-center gap-3">
          <span className="text-xs font-bold uppercase tracking-[0.3em] text-[#8e8a84] bg-[#fdf2f2] px-3 py-1 rounded-full">Sacred Healing</span>
        </div>
        <h2 className="font-serif text-5xl md:text-7xl font-light tracking-tight">Womb Work</h2>
        <p className="text-[#6b6863] text-xl max-w-2xl leading-relaxed">
      ...
      <section className="space-y-8">
        <div className="space-y-2">
          ...
          <p className="text-[#6b6863]">Understanding the cyclical nature of your body and energy.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {WOMB_SEASONS.map((season, idx) => (
            <div key={idx} className="bg-white border border-[#eeeae4] p-6 rounded-[2rem] space-y-4 hover:border-pink-200 transition-colors group">
              <div className="space-y-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-6">
          <h3 className="text-xs font-bold uppercase tracking-widest text-[#8e8a84]">Deeper Insights</h3>
          <div className="grid gap-4">
            <div className="bg-white border border-[#eeeae4] p-8 rounded-[2.5rem] flex items-center justify-between group cursor-pointer hover:border-pink-200 transition-all">
              ...
              <ChevronRight size={24} className="text-[#8e8a84] group-hover:translate-x-2 transition-transform" />
            </div>
            <div className="bg-white border border-[#eeeae4] p-8 rounded-[2.5rem] flex items-center justify-between group cursor-pointer hover:border-pink-200 transition-all">
              <div className="flex items-center gap-6">
                <div className="w-14 h-14 rounded-2xl bg-pink-50 flex items-center justify-center text-pink-400 group-hover:scale-110 transition-transform">
        ...
        <div className="bg-[#f5f2ed] p-8 rounded-[2.5rem] space-y-6">
          <h3 className="text-xs font-bold uppercase tracking-widest text-[#8e8a84]">A Note on Healing</h3>
          <p className="text-sm text-[#6b6863] leading-relaxed italic font-serif">
          ...
          <div className="pt-4 border-t border-[#eeeae4]">
            <p className="text-[10px] text-[#8e8a84] uppercase tracking-widest leading-relaxed">
  ...
  return (
    ...
    >
      <header className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-xs font-bold uppercase tracking-[0.3em] text-[#8e8a84] bg-[#f5f2ed] px-3 py-1 rounded-full">Spiritual Nourishment</span>
          </div>
          <div className="flex items-center gap-3 bg-white/80 backdrop-blur px-4 py-2 rounded-full border border-[#eeeae4] shadow-sm">
            <div className="flex flex-col items-end">
              <span className="text-[10px] uppercase tracking-widest text-[#8e8a84] font-bold">Reading Ambience</span>
              <div className="text-xs font-medium text-[#5A5A40] mt-1 text-right">
                YouTube Ambience
          ...
          <button
            ...
            className={cn(
              ...
              filter === cat
                ...
                : "bg-white border border-[#eeeae4] text-[#6b6863] hover:border-[#5A5A40]"
            )}
          >
            {cat}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {filteredVerses.map((v, idx) => (
          <motion.div
            ...
            className="bg-white border border-[#eeeae4] p-8 rounded-[2rem] space-y-6 hover:shadow-lg transition-shadow group"
          >
            <div className="flex items-center justify-between">
              <div className="w-8 h-8 rounded-full bg-[#f5f2ed] flex items-center justify-center text-[#5A5A40]">
                <BookOpen size={16} />
              </div>
              <span className="text-[10px] font-bold uppercase tracking-widest text-[#8e8a84]">{(v as any).category}</span>
            </div>
            <blockquote className="font-serif text-xl leading-relaxed text-[#2d2a26] group-hover:text-[#5A5A40] transition-colors">
              "{v.text}"
            </blockquote>
            <cite className="block text-sm font-medium text-[#8e8a84] not-italic">— {v.reference}</cite>
          </motion.div>
        ))}
      </div>

      <div className="bg-[#5A5A40] rounded-[3rem] p-12 text-white text-center space-y-6 shadow-2xl shadow-[#5A5A40]/30">
        ...
        <button
          ...
          className="px-8 py-3 bg-white text-[#5A5A40] rounded-full font-semibold hover:bg-[#f5f2ed] transition-colors"
        >
          Read Full Bible
        </button>
      </div>
...
function HealingSpaceView() {
  return (
    ...
    >
      <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-lg text-[#5A5A40] mb-4">
        <Heart size={40} />
      </div>
      ...
      <div className="flex gap-4">
        <button className="px-10 py-4 bg-[#5A5A40] text-white rounded-full font-semibold shadow-xl">Join Live Session</button>
        <button className="px-10 py-4 border border-[#5A5A40] text-[#5A5A40] rounded-full font-semibold">Request Prayer</button>
      </div>
